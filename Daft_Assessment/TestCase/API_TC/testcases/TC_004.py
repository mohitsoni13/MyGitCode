'''
TC_004 - Remove the MCA Address (required parameter) from JSON
'''
import sys
import logging
import traceback
...
'''
Remove the MCA address from JSON
Verify the error response message
'''
TEST_CASE_NAME = "Remove the MCA Address (required parameter) from JSON"
...
class TC_004():
    ...
    def __init__(self):
        ''' __init__ function '''        
        print TEST_SCRIPT+":Remove the MCA Address (required parameter) from JSON."
    ...
    def init(self):
        ...
        self.execute_msg = ""
        self.error_output = {"error":{"errors":[{"domain":"geolocation","reason":"invalidRequest","message":"Bad Request"}],"code":400,"message":"Bad Request"}}
    ...
    def execute(self):
        ...
        try: 
            self.urlResult = self.apihelperobj.getResponseMsg(config.url, jsonpara=config.removedMcaAddress)
            if self.urlResult:
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.debug("TC_004 test case: Exception: " + REPLACES)
