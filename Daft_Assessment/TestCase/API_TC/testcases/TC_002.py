'''
TC_002 - Removed all options parameters and check the response message
'''
import sys
import logging
import traceback
...
'''
In JSON remove all options parameters and pass it in URL
Verify the response message
'''
TEST_CASE_NAME = "Removed all options parameters and check the response message"
...
class TC_002():
    ...
    def __init__(self):
        ''' __init__ function '''        
        print TEST_SCRIPT+":Removed all options parameters and check the response message."
    ...
    def execute(self):
        ...
        try: 
            self.urlResult = self.apihelperobj.getResponseMsg(config.url,config.removedAllOptions)
            if self.urlResult:
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.debug("TC_002 test case: Exception: " + REPLACES)
